...
Description: 
    Tracking attack based on ML (Maximum Likelihood) estimation for transition 
    matrices and the Viterbi algorithm [Shokri+, S&P11]. 
    ...
    2) Re-identify each trace by choosing a user with the highest posterior. 
       (Compute a posterior via the Forward algorithm [Shokri+, S&P11].)
    3) Infer each location via the Viterbi algorithm.

Reference:
    R.Shokri et al., Quantifying Location Privacy, IEEE S&P, 2011.

Usage:
    R2-MLTrans-Bayes.py [Training Trace (in)] [Anonymized Trace (in)] [Estimated Table (out)]
"""
import numpy as np
import math
import csv
...
#sys.argv = ["R2-MLTrans-Bayes.py", "../Data/traintraces_TK.csv", "../Data_Anonymized_Shuffled/testtraces_TK_A1.csv", "../Data_Reidentified/etable_TK_A1-R2.csv"]
sys.argv = ["R2-MLTrans-Bayes.py", "../Data/testtraces_TK.csv", "../Data_Anonymized_Shuffled/testtraces_TK_A1.csv", "../Data_Reidentified/etable_TK_A1-R2.csv"]
if len(sys.argv) < 3:
...
# [output1]: visit_vec ({(user_id, reg_id): log_visit_prob})
# [output2]: trans_mat ({(user_id, reg_id, reg_id_to): log_trans_prob})
def TrainVisitTrans():
    # Initialization
    visit_vec = {}
    trans_mat = {}
...
#        user_id = int(lst[0])
    ...
    for (user_id, reg_id), counts in sorted(visit_vec.items()):
        visit_vec[user_id, reg_id] = math.log(counts / visit_sum[user_id])
    ...
    for (user_id, reg_id, reg_id_to), counts in sorted(trans_mat.items()):
        trans_mat[user_id, reg_id, reg_id_to] = math.log(counts / trans_sum[user_id, reg_id])

    return visit_vec, trans_mat

###################### Bayesian re-identification attack ######################
# [input1]: visit_vec ({(user_id, reg_id): log_visit_prob})
# [input2]: trans_mat ({(user_id, reg_id, reg_id_to): log_trans_prob})
        ...
        # For a non-new user
            ...
            for i in range(UserNum):
                if (i, reg_id_pre, reg_id) in trans_mat:
                    logpost[i] += trans_mat[i, reg_id_pre, reg_id]
                else:
                    logpost[i] += logdelta
