...
Description: 
    Tracking attack based on visit-probability vectors. 
...
Usage:
    T2-VisitProb.py [Training Trace (in)] [Anonymized Trace (in)] [Estimated Trace (out)]
"""
import numpy as np
import math
import csv
...
#sys.argv = ["T2-VisitProb.py", "../Data/testtraces_TK.csv", "../Data_Anonymized_Shuffled/testtraces_TK_A2-0-0-0.5.csv", "../Data_Tracked/etraces_TK_A2-T2.csv"]
if len(sys.argv) < 3:
...
#        pse_id = int(lst[0])
        ...
        # Choose a location with the highest posterior --> est_trace[(user_id,time_id)]
        # Noise
        if reg_id_num == 1 and reg_id_lst[0] != "*":
...
#            est_trace[(user_id,time_id)] = int(reg_id_lst[0])
        ...
        # Generalization
            ...
            # Find a location with the highest posterior --> max_r
            max_likeli = 0
            max_r = 0
            for r in range(reg_id_num):
                # region ID --> reg_id
#                reg_id = int(reg_id_lst[r])
                reg_id = int(reg_id_lst[r])-1
                if (user_id, reg_id) in visit_vec and max_likeli < visit_vec[user_id, reg_id]:
                    max_likeli = visit_vec[user_id, reg_id]
                    max_r = r
            # Region ID with the highest posterior --> est_trace[(user_id,time_id)]
#            est_trace[(user_id,time_id)] = int(reg_id_lst[max_r])
            est_trace[(user_id,time_id)] = int(reg_id_lst[max_r])-1
        # location hiding (deletion)
        else:
            # Find a location with the highest posterior --> max_r
            max_likeli = 0
            max_reg_id = 0
            for reg_id in range(r_num):
                if (user_id, reg_id) in visit_vec and max_likeli < visit_vec[user_id, reg_id]:
                    max_likeli = visit_vec[user_id, reg_id]
                    max_reg_id = reg_id
            # Region ID with the highest posterior --> est_trace[(user_id,time_id)]
            est_trace[(user_id,time_id)] = max_reg_id
