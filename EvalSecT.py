...
########## Calculate a security score (tracking) between two regions ##########
# [input1]: reg_id1
# [input2]: reg_id2
# [input3]: xc
# [input4]: yc
def CalTScore(reg_id1, reg_id2, xc, yc):
    ...
    if dist_km > MinDisZeroScore:
        ...
        tscore = 1 - dist_km / MinDisZeroScore
...
# Calculate the average security score (tracking) --> avg_tscore
avg_tscore = 0
for (user_id, time_id) in test_trace:
    reg_id1 = test_trace[(user_id, time_id)]
    reg_id2 = est_trace[(user_id, time_id)]
    avg_tscore += CalTScore(reg_id1, reg_id2, xc, yc)
avg_tscore /= len(test_trace)
